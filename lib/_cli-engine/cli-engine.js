...
class CLIEngine {
    ...
    constructor(providedOptions) {
         ...
         * @type {FileEnumerator}
         */
        this.enumerator = new FileEnumerator({
        ...
        for (const rulesdir of options.rulePaths || []) {
            ...
            this.linter.defineRules(loadRules(rulesdir, options.cwd));
        }
    ...
    executeOnText(text, filename, warnIgnored) {
        ...
        if (
            resolvedFilename &&
            enumerator.ignoredPaths.contains(resolvedFilename)
        ) {
            if (warnIgnored) {
     ...
     */
    _getConfigArrayForFile(filePath) {
        const baseConfig = ConfigArrayFactory.create(this.options.baseConfig);
        const intermediateConfig = this.options.useEslintrc
            ? ConfigArrayFactory.loadInAncestors(
                filePath,
                {
                    parent: baseConfig,
                    usePersonalEslintrc: !this.options.configFile
                }
            )
            : baseConfig;
        const config = this.enumerator.finalizeConfigArray(intermediateConfig);
     ...
     */
    getFormatter(format) {
        return loadFormatter(format || "stylish", this.options.cwd);
    }
    ...
    getConfigForFile(filePath) {
        ...
        const relativePath = path.relative(this.options.cwd, absolutePath);

        return this
            ._getConfigArrayForFile(absolutePath)
    ...
    isPathIgnored(filePath) {
        return this.enumerator.ignoredPaths.contains(filePath);
    }
    ...
    addPlugin(name, pluginObject) {
        ...
        for (const [ruleId, rule] of Object.entries(pluginObject.rules || {})) {
            linter.defineRule(`${name}/${ruleId}`, rule);
        }
        for (const [envId, env] of Object.entries(pluginObject.environments || {})) {
            linter.environments.define(`${name}/${envId}`, env);
        }

        // TODO: give FileEnumerator the plugin to load it.
    }
