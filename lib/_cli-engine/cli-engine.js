...
class CLIEngine {
    ...
    executeOnFiles(patterns) {
        ...
        for (const { config, filePath, flag } of enumerator.iterateFiles(patterns)) {
            ...
            // Check plugin uniqueness.
            if (!checkedConfigs.has(config)) {
                checkedConfigs.add(config);

                for (const [id, plugin] of config.getAllPlugins()) {
                    const existingPlugin = plugins.get(id);

                    if (existingPlugin && existingPlugin.definition !== plugin.definition) {
                        const error = new Error(`Plugin "${id}" conflicted.`);

                        error.messageTemplate = "plugin-conflict";
                        error.messageData = {
                            pluginName: id,
                            plugin1: existingPlugin,
                            plugin2: plugin
                        };
