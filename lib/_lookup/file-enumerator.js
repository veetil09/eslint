...
const { escapeRegExp, uniqBy } = require("lodash");
...
class FileEnumerator {
     ...
     * @param {FileEnumeratorOptions} options The options to iterate files.
     * @returns {FileAndConfig[]} The found files.
     */
    static iterateFiles(patterns, options) {
        return uniqBy(
            Array.from(new FileEnumerator(options).iterateFiles(patterns)),
    ...
    constructor({
         ...
         * @private
         */
        this.baseConfigArray = ConfigArray.create(
            baseConfig,
    ...
    *iterateFilesInDirectoryRecursive(
    ...
    loadConfigInAncestors(directoryPath) {
        if (this.useEslintrc) {
            return ConfigArray.loadInAncestors(
                directoryPath,
                {
    ...
    loadConfigOnDirectory(directoryPath, parentConfigArray) {
        if (this.useEslintrc) {
            return ConfigArray.loadOnDirectory(
                directoryPath,
    ...
    finalizeConfigArray(parentConfigArray) {
        ...
        if (!finalConfigArray) {
            ...
            // others
            finalConfigArray = ConfigArray.create(
                this.cliConfigData,
                {
