...
module.exports = class Linter {
     ...
     */
    _verifyWithConfigArray(text, configArray, options) {

        /*
         * TODO: implement https://github.com/eslint/rfcs/pull/3 here.
         */

        const config = configArray.extractConfig(options.filename);
        const rules = ruleMaps.get(this);
        const envs = this.environments;

        // Setup rules and envs of depending plugins.
        for (const [pluginId, { definition: plugin }] of Object.entries(config.plugins)) {
            for (const [ruleId, rule] of Object.entries(plugin.rules || {})) {
                rules.define(`${pluginId}/${ruleId}`, rule);
            }
            for (const [envId, env] of Object.entries(plugin.environments || {})) {
                envs.define(`${pluginId}/${envId}`, env);
            }
        }

        // Setup parser.
        if (config.parser) {
            this.defineParser(config.parser.id, config.parser.definition);
            config.parser = config.parser.id;
        }
        ...
        return this.verify(
            text,
            config,
            { ...options, ...config.processor.definition }
        );
    }
